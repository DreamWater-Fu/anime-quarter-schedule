...
### 2026-07-29：前端 Material 风格与追番时间轴优化

- 前端改为 Material Design 取向的工具页风格：使用 surface/container 分层、圆角按钮、状态层 hover、轻量 elevation 和更清晰的控件分组。
- 按用户要求移除统计列表中的“数据状态”列，并移除前端可见的数据状态筛选；数据审查统计仍保留在更新完成弹窗中。
- 追番列表参考 Bilibili 追番时间表的信息节奏，改为北京时间时间轴 + 番剧卡片 + 评分胶囊布局。
- 小屏顶部工具栏不再 sticky，避免滚动时占用过多屏幕；统计列表保留卡片式行展示，避免移动端依赖横向滚动。
- 封面外链加载失败时显示字首占位，避免破图和 alt 文本挤压列表。
...
- 验证：`npm run lint`、`npm run test`、`npm run build` 通过；Playwright 预览桌面与 390px 移动端，追番列表移动端横向溢出检查为 `false`。
